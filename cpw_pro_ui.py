...
- 下载 / ffmpeg / start_server / start_client 见 `cpw_worker.py`；下载·抽轨·转写/VAD 线程编排见 `cw_transcribe.py`。
- 链接清洗与 ANSI 剔除见 `cpw_textutil.py`；CTk 外观与示波配色见 `cpw_theme.py`。
"""
...
from cpw_textutil import normalize_url, strip_ansi
from cpw_theme import apply_ctk_defaults, scope_canvas_bg_and_stroke as _scope_canvas_bg_and_stroke
from cpw_progress import (
...
from llm_client import stream_chat, build_messages
from media_utils import AudioEngine, parse_srt
from timestamp_quality import analyze_timestamp_quality, format_quality_report
...
_APP_CONFIG_PATH = Path(__file__).parent / "config.json"
...
class App(ctk.CTk):
    ...
    # ---------- pipeline hooks（后台线程通过 cw_transcribe 调用）----------
